...
export function GameScreen({ level, onBack, onNextLevel, onLevelComplete }: Props) {
  ...
  // Play merge sound when groups decrease
  ...
  // Play select sound on new selection
  ...
  // Click handler with sound (for click-swap mode)
  ...
  // Report completion once + play victory
  ...
  useEffect(() => {
    ...
    if (!isComplete) completionReportedRef.current = false;
  }, [isComplete, play, onLevelComplete, level.id, moves, elapsedTime]);

  // Force re-render on sound toggle
    ...
    <div className={styles.screen}>
      ...
      <div className={styles.timerBar}>
        <span
          ...
          aria-live="polite"
          aria-label={`Tiempo: ${formatTime(elapsedTime)}`}
        >
